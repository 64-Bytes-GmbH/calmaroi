      ...
      success: function (data) {
        if (data.status === 'success') {
          if (isEditing) {
            ...
            row.find('td:eq(0)').text(data.data.abschluss);
            row.find('td:eq(1)').text(data.data.institution);
            row.find('td:eq(2)').text(`${data.data.start_date} - ${data.data.end_date}`);
          } else {
            ...
            const newRow = $(`
              <tr id="row-${rowIndex}" data-id="${data.data.id}">
                <td class="md:px-6 md:py-3 py-2 pr-1 whitespace-nowrap text-sm font-medium text-gray-900">${data.data.abschluss}</td>
                <td class="md:px-6 md:py-3 py-2 pr-1 whitespace-nowrap text-sm text-gray-500">${data.data.institution}</td>
                <td class="md:px-6 md:py-3 py-2 pr-1 whitespace-nowrap text-sm text-gray-500">${data.data.start_date} - ${data.data.end_date}</td>
                <td class="md:px-6 md:py-3 py-2 pr-1 whitespace-nowrap text-right text-sm font-medium relative">
